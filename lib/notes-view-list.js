...
export default class NotesViewList {
  ...
  elementForItem (item) {
    ...
    if (query.length > 0) {
      const itemtext = matter(item.body).content
      const lcitemtext = itemtext.toLowerCase()
      const appearancePos = lcitemtext.indexOf(query.toLowerCase())
      if (appearancePos >= 0) {
        const showfrom = Math.max(0, appearancePos - 25)
        const preamble = lcitemtext.slice(showfrom, appearancePos)
        const followtxt = lcitemtext.slice(appearancePos + query.length, showfrom + 100)
        const matchtxt = itemtext.slice(appearancePos, appearancePos + query.length)
        secondary.innerHTML = preamble + "<span class=\"highlight-info\">" + matchtxt + "</span>" + followtxt
      }
    }
